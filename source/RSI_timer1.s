...
@;=                                                           	    	=
@;=== Programador tarea 2F: ivan.morillas@estudiants.urv.cat				  ===
@;=                                                       	        	=
...
.data
	...
	divFreq1: .hword	-5727		@;divisor de frecuencia para timer 1
									@;Div_Frec = -(Frec_Ent/Frec_Sor)  
									@;-(523655.96875/(32/0.35)) = -5727.487159
...
activa_timer1:
		...
		strh r0, [r1]
		cmp r0, #0
		bne .LFinal_activar_timer1			@; si escSen es 0, fijar la variable escFac a 1,0
		ldr r2, =escFac
		mov r1, #1
		mov r1, r1, lsl #8					@; formato decimal de coma fija 0.8.8
		strh r1, [r2]
		mov r2, r1
		bl SPR_fijarEscalado				@; actualizar el factor de escalado actual sobre los parametros PA y PD del grupo 0
	.LFinal_activar_timer1:
		mov r0, #0
		ldr r1, =escNum						@; fijar la variable escNum a 0
		strh r0, [r1]
		mov r0, #1
		...
		strh r0, [r1]
		ldr r1, =0x04000106					@; 0400 0106 TIMER1_CR Registro de control del timer1
...
desactiva_timer1:
		...
		ldr r1, =0x04000106
		ldrh r0, [r1]
		bic r0, #0x80						@; desactivar el timer1 a traves de su registro E/S de control
		strh r0, [r1]
		mov r0, #0
...
rsi_timer1:
		...
		ldr r1, =escNum
		ldrh r0, [r1]
		add r0, #1							@; aumentar la variable escNum + 1
		strh r0, [r1]
		...
		ldrh r2, [r3]
		ldrh r0, [r1]
		cmp r0, #0							@; aumentar o disminuir el factor de escalado
		mov r4, #10
		subne r2, r4
		addeq r2, r4
		strh r2, [r3]
